...
class MyThread_sweep(QThread):
    ...
    def run(self):
        ...
        while(True):
            if not warning:
                self.sweepLedsSignal.emit(step)
                step = step + 1
                if step == 4:
                    step = 0
                time.sleep(0.5)
...
class Ui_MainWindow(object):
    ...
    def sweep_leds(self,val):
        ...
        if val == 1:
            self.set4leds("white","orange","white","white")
        if val == 2:
            self.set4leds("white","white","orange","white")
        if val == 3:
            self.set4leds("white","white","white","orange")
        if val == 4:
            self.set4leds("white", "white", "white", "white")
    ...
    def valuechange(self):
        ...
        if progress_bar_value < actual_value:
            self.change_pb_up_value(actual_value)
        if progress_bar_value > actual_value:
            self.change_pb_down_value(actual_value)
    ...
    def change_pb_down_value(self,value):
        global progress_bar_value
        while progress_bar_value > value:
    ...
    def warningLightsButton(self):
        ''' complete with necesarry code '''
        global warning
        if not hasattr(self,"thread_warning"):
            warning = True
            ...
            if hasattr(self,"thread"):
                self.sweep_leds(4) # wait until all the lights stop
        else:
            warning = False
            self.thread_warning.terminate()
            self.setWarningLights("white","white","white","white")
    ...
    def fade_door_left(self):
        global flag_left_door, brightness_left_door
        flag_left_door = self.spinBox_left.value()
        brightness_left_door = 0
        while brightness_left_door < flag_left_door:
            brightness_left_door += 1
            self.left_door_slider.setValue(brightness_left_door)
        else:
            self.left_door_slider.setValue(brightness_left_door)
    ...
    def valuechange_left_slider(self):
        ...
        global flag_left_door,brightness_left_door
        flag_left_door = self.spinBox_left.value()
        brightness_left_door = self.left_door_slider.value()
        if brightness_left_door >= flag_left_door:
            self.left_door_slider.setValue(flag_left_door)
        if brightness_left_door == flag_left_door:
            self.setWarningLights("red",transparent,"red",transparent)
        else:
            contrast = (brightness_left_door / (flag_left_door + 0.01)) / 2
            color = f"rgba({contrast * 255 + 122},0,0,{contrast})"
            self.setWarningLights(color,transparent,color,transparent)

    # Open right door untill the obstacle is detected      
    def fade_door_right(self):
        global flag_right_door, brightness_right_door
        flag_right_door = self.spinBox_right.value()
        brightness_right_door = 0
        while brightness_right_door < flag_right_door:
            brightness_right_door += 1
            self.right_door_slider.setValue(brightness_right_door)
        else:
            self.right_door_slider.setValue(brightness_right_door)
    ...
    def valuechange_right_slider(self):
        ...
        global flag_right_door, brightness_right_door
        flag_right_door = self.spinBox_right.value()
        brightness_right_door = self.right_door_slider.value()
        if brightness_right_door >= flag_right_door:
            self.right_door_slider.setValue(flag_right_door)
        if brightness_right_door == flag_right_door:
            self.setWarningLights(transparent,"red", transparent,"red")
        else:
            contrast = (brightness_right_door / (flag_right_door + 0.01)) / 2
            color = f"rgba({contrast * 255 + 122},0,0,{contrast})"
            self.setWarningLights(transparent, color, transparent, color)
